...
class TestWatcherIntegration:
    ...
    async def test_memory_cleanup(self) -> None:
        """Test that seen_deals memory is cleaned up properly."""
        watcher = DealWatcher()
        
        # Add many deals to trigger cleanup
        with patch('time.time') as mock_time, \
             patch('app.watcher.extract.deal_key') as mock_key:
            
            mock_time.return_value = 1000.0
            
            # Add 105 deals (more than 100 limit)
            for i in range(105):
                mock_key.return_value = f"wine_{i}"
                deal_details = DealDetails(wine_name=f"Wine {i}", deal_price=float(i + 1))  # +1 to avoid 0
                
                with patch('app.watcher.enrich_deal'), \
                     patch('app.watcher.send_telegram_message'):
                    await watcher._process_deal_details(deal_details, "test")
            
            # Should have triggered cleanup and kept only recent entries
